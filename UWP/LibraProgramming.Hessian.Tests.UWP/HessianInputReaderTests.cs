...
namespace LibraProgramming.Hessian.Tests.UWP
{
    ...
    public class HessianInputReaderTests
    {
        ...
        {
            AssertInput(value => Assert.AreEqual(Single.MinValue, Convert.ToDouble(value)), new byte[] {0x5F, 0xFF, 0x7F, 0xFF, 0xFF}, reader => reader.ReadDouble());
            AssertInput(value => Assert.AreEqual(Single.MaxValue, Convert.ToDouble(value)), new byte[] {0x5F, 0x7F, 0x7F, 0xFF, 0xFF}, reader => reader.ReadDouble());
        }
        ...
        private static void AssertInput(byte[] expected, byte[] input, Func<HessianInputReader, byte[]> action)
        {
            ...
            {
                if (expected.Length != temp.Length)
                {
                    Assert.Fail();
                }
